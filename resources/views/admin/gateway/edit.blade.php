            ...
            <div class="card">
                <form action="{{ route('admin.gateway.automatic.update', $gateway->code) }}" method="POST"
                        ...
                        <div class="payment-method-item">
                            <div class="payment-method-header row">
                                <div class="col-md-6 col-sm-12">
                                    <img class="img-thumbnail mb-1" src="{{ getImage(imagePath()['gateway']['path'].'/'. $gateway->image,imagePath()['gateway']['size']) }}"/>
                                    <input class="form-control" name="image" type="file" id="image" accept=".png, .jpg, .jpeg" />
                                </div>
                                <div class="col-md-6 col-sm-12 mt-1">
                                    <div class="d-flex justify-content-start">
                                        <h3 class="title">{{ __($gateway->name) }}</h3>
                                        <div class="input-group ms-1 d-flex flex-wrap justify-content-start">
                                            <select class="newCurrencyVal form-select">
                                                ...
                                                @empty
                                                    <option value="">{{ __('locale.No available currency support')}}</option>
                                                @endforelse

                                            </select>
                                                <button type="button" class="btn btn-primary newCurrencyBtn" data-crypto="{{ $gateway->crypto }}" data-name="{{ $gateway->name }}">{{ __('locale.Add new')}}
                                                </button>
                                        </div>
                                    </div>
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <div>
                                                        <div class="input-group-text">
                                                            <span class="copyInput" data-bs-toggle="tooltip" title="{{ __('locale.Copy')}}"><i class="bi bi-clipboard"></i></span>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div class="payment-method-body mt-2">
                                <h4 class="mb-3">{{ __('locale.Global Setting for')}} {{ __($gateway->name) }}</h4>
                                <div class="row">
                        ...
                        @isset($gateway->currencies)
                            @foreach($gateway->currencies as $gateway_currency)
                                <input type="hidden" name="currency[{{ $currency_idx }}][symbol]"
                                    ...
                                    <div class="row mt-2">
                                        <div class="col-md-6 col-sm-12">
                                            <img class="img-thumbnail mb-1 mt-1" src="{{getImage(imagePath()['gateway']['path'].'/'.$gateway_currency->image,imagePath()['gateway']['size'])}}">
                                            <input class="form-control" name="currency[{{ $currency_idx }}][image]" type="file" id="mage{{ $currency_idx }}" accept=".png, .jpg, .jpeg" required="" />
                                        </div>
                                        <div class="col-md-6 col-sm-12">
                                            <h4>{{ __($gateway->name) }} - {{__($gateway_currency->currency)}}</h4>
                                            <input type="text" class="form-control" placeholder="{{ __('locale.Name for Gateway')}}" name="currency[{{ $currency_idx }}][name]" value="{{ $gateway_currency->name }}" required/>
                                            <button type="button" class="btn btn-danger deleteBtn mt-1" data-id="{{ $gateway_currency->id }}" data-name="{{ $gateway_currency->currencyIdentifier() }}">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12 col-md-6 col-lg-6 col-xl-4">
                                                <div class="card border-primary mt-2">
                                                    ...
                                                    <div class="card-body">
                                                        <label class="w-100 mt-1">{{ __('locale.Minimum Amount')}} <span class="text-danger">*</span></label>
                                                        ...
                                                        <label class="w-100">{{ __('locale.Maximum Amount')}} <span class="text-danger">*</span></label>
                                                        <div class="input-group">
                                                            <div class="input-group-text">{{ __($general->cur_text) }}</div>
                                            ...
                                            <div class="col-sm-12 col-md-6 col-lg-6 col-xl-4">
                                                <div class="card border-primary mt-2">
                                                    ...
                                                    <div class="card-body">
                                                        <label class="w-100 mt-1">{{ __('locale.Fixed Charge')}} <span class="text-danger">*</span></label>
                                                        <div class="input-group mb-1">
                                                            <div class="input-group-text">{{ __($general->cur_text) }}</div>
                                                        ...
                                                        <label class="w-100">{{ __('locale.Percent Charge')}} <span class="text-danger">*</span></label>
                                                        <div class="input-group">
                    ...
                    <!-- payment-method-item end -->
                        ...
                        <div class="payment-method-item child-item newMethodCurrency d-none p-2">
                            <input disabled type="hidden" name="currency[{{ $currency_idx }}][symbol]"
                                   class="currencySymbol">
                            <div class="row">
                                <div class="col-lg-6 col-md-6 col-sm-12">
                                    <img class="img-thumbnail mb-1 mt-1" src="{{getImage(imagePath()['gateway']['path'],imagePath()['gateway']['size'])}}">
                                    <input disabled type="file" accept=".png, .jpg, .jpeg" class="form-control" id="image{{ $currency_idx }}" name="currency[{{ $currency_idx }}][image]"/>
                                </div>
                                <div class="col-lg-6 col-md-6 col-sm-12">
                                        <div class="col">
                                            <h4 class="mb-1" id="payment_currency_name">{{ __('locale.Name')}}</h4>
                                            <input disabled type="text" class="form-control" placeholder="{{ __('locale.Name for Gateway')}}" name="currency[{{ $currency_idx }}][name]" required/>
                                        </div>
                                        <div class="col">
                                ...
                                <div class="row">
                                        ...
                                        <div class="card border-primary mt-2">
                                            ...
                                            <div class="card-body">
                                                <div class="input-group mb-3">
                                                    <label class="w-100">{{ __('locale.Minimum Amount')}} <span class="text-danger">*</span></label>
                                    ...
                                    <div class="col-sm-12 col-md-6 col-lg-6 col-xl-4">
                                        <div class="card border-primary mt-2">
                                            ...
                                            <div class="card-body">
                                                <div class="input-group mb-3">
                                                    <label class="w-100">{{ __('locale.Fixed Charge')}} <span class="text-danger">*</span></label>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('locale.Gateway Currency Remove Confirmation')}}</h5>
                ...
                <form action="{{ route('admin.gateway.automatic.remove', $gateway->code) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>{{ __('locale.Are you sure to delete')}} <span class="fw-bold name"></span> {{ __('locale.gateway currency?')}}</p>
                    </div>
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.gateway.automatic.index') }}" class="btn btn-primary"><i class="bi bi-chevron-left"></i> {{ __('locale.Go Back')}}</a>
@endpush
